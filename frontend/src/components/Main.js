            ...
            <ul className="cards">
              {
                cards.map((card) => { 
                  return (
                  <Card card={card} key={card._id} likes={card.likes.length} name={card.name} link={card.link} onCardClick={onCardClick} onCardLike={onCardLike} onConfirmCardDelete={onConfirmCardDelete} />
                )})}
            </ul>
        </main>
    );
